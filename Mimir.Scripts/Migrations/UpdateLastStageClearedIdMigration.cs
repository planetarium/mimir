...
public class UpdateLastStageClearedIdMigration
{
    ...
    public UpdateLastStageClearedIdMigration(IMongoDbService mongoDbService, ILogger<UpdateLastStageClearedIdMigration> logger)
    {
    ...
    public async Task<int> UpdateLastStageClearedIdAsync(string databaseName)
    {
        ...
        var collection = _mongoDbService.GetCollection<BsonDocument>("world_information");
        ...
        while (await cursor.MoveNextAsync())
        {
            ...
            {
                _logger.LogWarning("커서의 Current가 null입니다. 데이터베이스: {DatabaseName}", databaseName);
                continue;
            }
            ...
            foreach (var doc in cursor.Current)
            {
                var worldDictionary = doc.GetValue("Object", new BsonDocument())
                    .AsBsonDocument
                    .GetValue("WorldDictionary", new BsonDocument())
                    .AsBsonDocument;
                
                if (worldDictionary == null || worldDictionary.ElementCount == 0)
                {
                    _logger.LogInformation("WorldDictionary가 존재하지 않음, 건너뜀: {DocumentId}", doc["_id"]);
                    continue;
                }
                
                var worldKeys = worldDictionary.Names
                    .Where(key => int.TryParse(key, out var num) && num < 10000)
                    .Select(key => int.Parse(key))
                    .ToList();
                
                if (!worldKeys.Any())
                {
                    _logger.LogInformation("유효한 월드 키가 없음, 건너뜀: {DocumentId}", doc["_id"]);
                    continue;
                }
                ...
                {
                    var world = worldDictionary.GetValue(key.ToString(), new BsonDocument()).AsBsonDocument;
                ...
                {
                    _logger.LogInformation("StageClearedId가 -1이 아닌 월드가 없음, 건너뜀: {DocumentId}", doc["_id"]);
                    continue;
                }
                
                _logger.LogInformation("문서 ID: {DocumentId}, 월드 ID: {WorldId}, 가장 높은 StageClearedId: {StageClearedId}", 
                    doc["_id"], highestWorldId, highestStageClearedId);
                
                try
                {
                    var updateFilter = Builders<BsonDocument>.Filter.Eq("_id", doc["_id"]);
                    var update = Builders<BsonDocument>.Update
                        .Set("LastStageClearedId", highestStageClearedId)
                        .Set("Metadata.SchemaVersion", 2);
                    
                    collection.UpdateOne(updateFilter, update);
                    _logger.LogInformation("LastStageClearedId 업데이트 완료: {StageClearedId}, SchemaVersion: 2", highestStageClearedId);
                    count++;
                }
                catch (Exception ex)
                {
        ...
        _logger.LogInformation("{DatabaseName} 데이터베이스 업데이트 완료. {Count}개 문서 수정됨.", databaseName, count);
        return count;
    }
    ...
    public async Task<int> ExecuteAsync()
    {
        try
        {
            ...
            _logger.LogInformation("모든 업데이트 완료. 총 {TotalCount}개 문서 수정됨.", odinCount + heimdallCount);
