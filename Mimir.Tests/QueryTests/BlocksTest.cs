...
public class BlocksTest
{
    ...
    public async Task GetBlocks_Returns_PaginatedBlocks()
    {
        ...
        mockRepo
            .Setup(repo => repo.Get(It.IsAny<BlockFilter?>()))
    ...
    [Fact]
    public async Task GetBlocks_WithMinerFilter_Returns_CorrectBlocks()
    {
        var mockRepo = new Mock<IBlockRepository>();
        var minerAddress = new Address("0x0000000000000000000000000000000000000003");
        var blocks = new List<BlockDocument>
        {
            new BlockDocument(
                6494625,
                "blockHash1",
                new Lib9c.Models.Block.Block
                {
                    Index = 6494625,
                    Hash = "blockHash1",
                    Miner = minerAddress,
        ...
        var query = $$"""
                    query {
                      blocks(filter: { miner: "{{minerAddress.ToHex()}}" }) {
                        items {
                          id
                          object {
                            index
